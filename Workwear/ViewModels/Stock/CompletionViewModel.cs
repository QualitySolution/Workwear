	...
	public class CompletionViewModel: EntityDialogViewModelBase<Completion>
	{
			...
			IValidator validator = null) : base(uowBuilder, unitOfWorkFactory, navigation, validator)
		{
			...
			if(Entity.SourceWarehouse == null)
				Entity.SourceWarehouse = stockRepository.GetDefaultWarehouse(UoW, featuresService, autofacScope.Resolve<IUserService>().CurrentUserId);
			
			WarehouseExpenseEntryViewModel = entryBuilder.ForProperty(x => x.SourceWarehouse).MakeByType().Finish();
			
			if(Entity.ReceiptWarehouse == null)
				Entity.ReceiptWarehouse = stockRepository.GetDefaultWarehouse(UoW, featuresService, autofacScope.Resolve<IUserService>().CurrentUserId);

			WarehouseReceiptEntryViewModel = entryBuilder.ForProperty(x => x.ReceiptWarehouse).MakeByType().Finish();
		}
		...
		public void AddSourceItems(){
			var selectJournal = MainClass.MainWin.NavigationManager.OpenViewModel<StockBalanceJournalViewModel>(this, QS.Navigation.OpenPageOptions.AsSlave);
			if(Entity.SourceWarehouse != null) {
		...
		public void AddResultItems() {
			var selectJournal = MainClass.MainWin.NavigationManager.OpenViewModel<NomenclatureJournalViewModel>(this, QS.Navigation.OpenPageOptions.AsSlave);
