		...
		void CreateTable()
		{
			...
			treeItems.ColumnsConfig = Gamma.GtkWidgets.ColumnsConfigFactory.Create<ExpenseItem>()
				...
				.AddColumn("Размер")
					...
					.DynamicFillListFunc(x => SizeService.GetSize(ViewModel.UowOfDialog, x.Nomenclature.Type.SizeType,true).ToList())
					.AddSetter((c, n) => c.Editable = n.Nomenclature?.Type?.SizeType != null)
				.AddColumn("Рост")
					.AddComboRenderer(x => x.Height).SetDisplayFunc(x => x.Name)
					.DynamicFillListFunc(x => SizeService.GetSize(ViewModel.UowOfDialog, x.Nomenclature.Type.HeightType, true).ToList())
