		...
		private void ConfigureDlg()
		{
			...
			ytreeviewItems.CreateFluentColumnsConfig<IssuanceSheetItem>()
				...
				.AddColumn("Размер")
					...
					.DynamicFillListFunc(x => SizeService.GetSize(ViewModel.UoW, x.Nomenclature?.Type?.SizeType, true).ToList())
					.AddSetter((c, n) => c.Editable = n.Nomenclature?.Type?.SizeType != null)
				.AddColumn("Рост")
					.AddComboRenderer(x => x.Height).SetDisplayFunc(x => x.Name)
				.DynamicFillListFunc(x => SizeService.GetSize(ViewModel.UoW, x.Nomenclature?.Type?.HeightType, true).ToList())
