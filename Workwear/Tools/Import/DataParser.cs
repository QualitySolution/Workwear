...
namespace workwear.Tools.Import
{
	public class DataParser
	{
		...
		public DataParser(SubdivisionRepository subdivisionRepository = null, PostRepository postRepository = null)
		{
			...
			AddColumnName(DataType.Subdivision,
				"Подразделение"
				);
			AddColumnName(DataType.Post,
				"Должность"
				);
		...
		public EmployeeCard PrepareToSave(IUnitOfWork uow, SheetRow row)
		{
		...
		private void SetValue(IUnitOfWork uow, EmployeeCard employee, DataType dataType, string value)
		{
			if(String.IsNullOrWhiteSpace(value))
				return;
